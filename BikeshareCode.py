...
def city_input():
    ...
    '''
    print('Hello! I am Tanyi ! Let\'s explore some US bikeshare data!')
    print(' ')
...
<<<<<<< HEAD
    # Just the month of the year as indicated not including Decembers and other months
||||||| merged common ancestors
=======
    # There is no December and other month of the year for now. 
>>>>>>> documentation
    if m == 'month':
        ...
        while month.strip().lower() not in ['january', 'february', 'march', 'april', 'may', 'june','july','august','september','october','november','december']:
            month = input('\nPlease choose between January, February, March, April, May, or June July, August, september, October, Novemeber, December? Please type the full month name.\n')
...
def ride_duration(df):
    ...
    '''
    # df - dataframe returned from time_filters
    print('\n * Q4. What was the total traveling done for 2019/2020 , and what was the average time spent on each trip?')
    ...
    print ("\nThe total travel time on 2019/2020 " + total_days + " days \n")
    avg_ride_time = np.mean(df['Travel Time'])
    avg_days = str(avg_ride_time).split()[0]
    print("The average travel time on 2019/2020 was " + avg_days + " days \n")
